...
int initialize(char *diskname, char *size, char *mbr, char *boot, char *kernel)
{
	...
	if (ret == 0 && boot != NULL)
	{
		...
		if (bootFile == NULL)
		{
			if (bootFileType == NULL)
			{
				printf("Error: bootFileType is null\n");
			}
			else
			{
	...
	if (ret == 0)
	{
		if (buffer != NULL)
		{
			...
			while (writeSize < diskSize)
			{
				...
				if (disk != NULL && fwrite(buffer, chunkSize, 1, disk) != 1)
				{
		...
		{
			// Manejo del caso cuando buffer es NULL
			printf("Error: buffer is null\n");
			ret = 1; // Asigna un valor a ret para indicar el error
		}
	}
	...
	if (ret == 0)
	{
		...
		{
			// Manejo del caso cuando disk es NULL
			printf("Error: disk is null\n");
			ret = 1; // Asigna un valor a ret para indicar el error
		}
	}

	// Write the master boot record if it was specified by the caller.
	// Supongamos que `disk` y `mbrFile` son punteros FILE* definidos y asignados en algún lugar del código anterior
	// y `buffer` es un puntero definido y asignado en algún lugar del código anterior
	...
	// Supongamos que `bootFile` es un puntero FILE* y `bootFileType` es un puntero a una cadena definidos y asignados en algún lugar del código anterior
	...
	if (ret == 0 && bootFile != NULL)
	{
			...
			if (disk != NULL)
			{
				...
				for (;;)
				{
					if (buffer != NULL)
					{
						...
						// No es necesario else aquí, ya que chunkSize == 0 puede significar EOF o error
					}
				}
			}
			else
		...
		{
			// Manejo del caso cuando bootFileType es NULL
			printf("Error: bootFileType is null\n");
			ret = 1; // Asigna un valor a ret para indicar el error
		}
	}
	...
	{
		// Manejo del caso cuando bootFile es NULL
		printf("Error: bootFile is null\n");
		ret = 1; // Asigna un valor a ret para indicar el error
	}
...
void create(char *filename, unsigned long long maxsize)
{
    ...
    if (findfile(filename, &tempentry, &slot) == 0)
    {
        ...
        for (tint = 0; tint < 64; tint++)
        {
            char *temp_ptr = dir_copy + tint * 64;
            memcpy(&tempentry, temp_ptr, sizeof(struct BMFSEntry));
            pEntry = &tempentry;
            ...
            if (pEntry->FileName[0] == 0x00) // end of directory
            {
                ...
                if (first_free_entry == -1)
                    first_free_entry = tint; // there were no unused entries before, will use this one
                break;
            }
        ...
        char *temp_ptr = Directory + first_free_entry * 64;
        memcpy(&tempentry, temp_ptr, sizeof(struct BMFSEntry));
        pEntry = &tempentry;
        ...
        if (first_free_entry == num_used_entries && num_used_entries + 1 < 64)
        {
            ...
            char *temp_ptr = Directory + (num_used_entries + 1) * 64;
            memcpy(&tempentry, temp_ptr, sizeof(struct BMFSEntry));
            pEntry = &tempentry;
